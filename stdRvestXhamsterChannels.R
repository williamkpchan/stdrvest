...
pageTail=""
className = "a.channel-thumb-container__image-container-image, a.channel-thumb-container__info-title, .channel-thumb-container__info-videos"
...
for(i in 1:length(addr)){
 ...
 itemList = gsub(' class=\"channel-thumb-container__image-container-image.*? src', '><img class="lazy" data-src', itemList)
 itemList = gsub('<img src=\"https://stati.*?>', '', itemList)
 itemList = gsub('<a.*? class="channel-thumb-container__info-title".*?>\\n {1,}', '', itemList)
 itemList = gsub('\\n {1,}</a>', '<br>', itemList)
 itemList = gsub('</a>', '</a><br>', itemList)
 itemList = gsub('<a', '<div><a', itemList)

 itemList = gsub('<div class="channel-thumb-container__info-videos.*?\\n', '', itemList)
 itemList = gsub('\\n {1,}</div>', '</div>', itemList)
 itemList = gsub('^ {1,}', '', itemList)
