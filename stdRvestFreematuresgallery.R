...
#pageTail="/"
pageTail=""
className = ".th" # pornpic
titleName = "freematuresgallery"
totalPages= 27

theFilename = paste0("freematuresgallery.photo.", titleName, ".html")
allLinks = character()
wholePage = character()
...
for(i in 1:length(addr)-1){
 ...
 itemListIdx = grep("http://archivegalleries", itemList)
 itemList = itemList[itemListIdx]
 itemList = gsub('/newpics', 'http://freematuresgallery.com/newpics', itemList)
 itemList = gsub('img src', 'img class="lazy" data-src', itemList)
